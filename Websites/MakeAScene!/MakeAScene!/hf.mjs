...
async function main() {
  ...
  if (!genreData || !genreData.characterArc || !genreData.badDialogue || !genreData.badWriting) {
    console.error("Missing required data in genre file.");
    rl.close();
    return;
  }
  ...
  async function createAndFilterSegment(prompt) {
    let segment = await generateText(prompt);
    let revisions = 0;
    while (containsBadContent(segment, genreData.badDialogue) || containsBadContent(segment, genreData.badWriting)) {
      console.log("\n⚠️ Warning: The segment contains elements of bad dialogue or bad writing.");
      if (revisions >= 3) {
        console.log("Too many revisions. Moving on...");
        break;
      }
      ...
      if (userFeedback.toLowerCase() === 'y') {
        ...
        segment = revisedSegment; // Update the segment with the user's revision
        revisions++;
      } else {
        break;
      }
...
const introPrompt = `Write a ${toneChoice} introduction for a horror story featuring a ${characterChoice} in a ${settingChoice}. The story should start with tension and introduce a conflict. The pacing should be ${pacingChoice}.`;
